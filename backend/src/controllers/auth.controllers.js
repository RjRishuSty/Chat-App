...
const signupHandler = async (req, res) => {
  ...
  try {
    ...
    res
      .status(201)
      .json({ message: "Sign up successfully.", success: true, data: newUser });
  } catch (error) {
    res.status(500).json({
...
const loginHandler = async (req, res) => {
  ...
  try {
    ...
    if (!matchPassword)
      return res
        .status(400)
        .json({ message: "Increate password.", success: false });
    // Generate jsonwebToken...............
...
const profileUpdateHandler = async (req, res) => {
  ...
  if (!profilePic)
    return res
      .status(400)
      .json({ message: "Profile picture field is requried.", success: false });
